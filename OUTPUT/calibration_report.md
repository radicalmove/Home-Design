...
- sunroom_wraparound_glazing: photo_verified for sunroom_right_glazing, sunroom_top_window, sunroom_left_window
  Evidence: Exterior and interior sunroom photos confirm wraparound glazing on the front/angled and side faces; corrected pixel markers should remain on the current sunroom polygon.
...
- deck_doors_and_kitchen_window: photo_verified for deck_doors, kitchen_window
  Evidence: Exterior deck photo confirms the large deck door group and adjacent tall deck-side glazing. Interior kitchen photos confirm this is the kitchen/dining deck-side wall, but the old kitchen_window label is treated as deck-side dining/kitchen glazing.
...
- entrance_laundry_back_entry: photo_verified for entrance, laundry, entrance_deck_slider
  Evidence: Entrance/back-entry photos confirm a glazed deck-side entrance leading directly to the laundry, with laundry bench, appliances, sink, and exterior window visible.
...
- hallway_spine_and_room_connections: photo_verified for hallway, hallway_to_entrance_laundry, hallway_to_bathroom_sliding_door, hallway_to_office_door
  Evidence: Hallway photos confirm the internal circulation spine from the lounge/office side toward the entrance/laundry end. Bathroom photos confirm the sliding bathroom door off the hallway, and office photos confirm the office doorway relationship.
...
- master_bedroom_photo_context: photo_context_verified for master_bedroom, master_street_window, master_sunroom_window
  Evidence: Master bedroom photos provide current-condition context for the room and its two window relationships: street/front side and sunroom side. Window markers are approximate until offsets are measured.
...
- bedroom2_photo_context: photo_context_verified for bedroom_2, bedroom2_se_window
  Evidence: Bedroom 2 photos confirm current room context and the SE-facing exterior window side. Window marker is approximate until offsets are measured.
...
- external_site_driveway_garage_deck_rear_garden: photo_verified for property_boundary, upper_side_driveway, front_diagonal_path, rear_timber_deck, cottage_end_deck, garage_concrete_pad, garage_cottage_side_path, garage_shed, cottage, boundary_hedges_and_fences
  Evidence: Satellite and outside photos confirm the main site topology: street-side driveway along the upper side of the section, a diagonal front path near the sunroom/front lawn, the timber deck outside the kitchen/dining and entrance/laundry side, a small cottage-end deck, the larger concrete area in front of the garage/cottage line, the narrow concrete path along the garage/cottage side, the current side-by-side garage/cottage relationship near the top boundary, and tall boundary hedges/fences that affect shade and privacy. Open grass/lawn remains the default ground context except where hardscape is explicitly mapped. External photos supersede the satellite where the cottage replaced part of the rear garage footprint.
...
- Spaces promoted: 11
  ...
  Space: lounge (measured_dimensions_inferred_position; evidence: sunroom_lounge_slider)
...
- Features promoted: 15
  ...
  Feature: deck_side_dining_window (window; photo_verified; evidence: deck_doors_and_kitchen_window)
  ...
  Feature: toilet_frosted_window (window; photo_verified; evidence: toilet_laundry_connection)
  ...
  Feature: master_street_window (window; photo_context_approx; evidence: master_bedroom_photo_context)
  Feature: master_sunroom_window (window; photo_context_approx; evidence: master_bedroom_photo_context, sunroom_wraparound_glazing)
  Feature: office_se_window (window; photo_context_approx; evidence: office_photo_context)
  Feature: bathroom_se_window (window; photo_context_approx; evidence: bathroom_photo_context)
  Feature: bedroom2_se_window (window; photo_context_approx; evidence: bedroom2_photo_context)
