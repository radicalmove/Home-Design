<script lang="ts">
  ...
  import {
    structuralTransitionForScenario,
    type ProposedPlanFloor,
    type StructuralTransitionOpening,
  } from "./lib/designStructuralTransitions";
  ...
  const MAIN_ENTRY_CUTOUT_COLOUR = "#9a6438";
  ...
  function openingCutoutStroke(opening: StructuralTransitionOpening): string | undefined {
    if (opening.kind === "window") {
      return WINDOW_CUTOUT_COLOUR;
    }
  ...
  >
    <defs>
      ...
      <pattern id="plan-overlay-carpet" width="18" height="18" patternUnits="userSpaceOnUse">
        ...
        <path d="M2 5 H5 M8 11 H11 M13 7 H16 M1 15 H3" stroke="#c4b49a" stroke-width="0.55" opacity="0.68" />
        <path d="M5 2 H7 M10 4 H13 M14 14 H17" stroke="#fff8ee" stroke-width="0.45" opacity="0.5" />
      </pattern>
        ...
        {#each structuralTransition.openings as opening (opening.id)}
          <g class={`proposed-plan-opening proposed-plan-${opening.kind}`}>
            <line
              ...
              stroke={openingCutoutStroke(opening)}
