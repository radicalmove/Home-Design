...
describe("plan vector model", () => {
  ...
  it("returns a final vector plan for Design 2 structural changes", () => {
    ...
    expect(featureIds).toEqual(expect.arrayContaining([
      ...
      "future_day_room_north_window",
      "future_day_room_east_window",
      "future_day_room_frosted_east_window",
      "future_kitchen_day_room_opening",
      "future_day_room_wide_opening",
      "relocated_wc_door",
    ]));
    ...
    expect(model.features.find((feature) => feature.id === "future_day_room_east_window")?.shape).toEqual({
      kind: "polygon",
      points: [
        [952.4, 514.3],
        [960.4, 514.3],
