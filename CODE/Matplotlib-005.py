...
png_path = out_dir / "clean_traced_site_plan_v36_polished_internal_overlay.png"
svg_path = out_dir / "clean_traced_site_plan_v36_polished_internal_overlay.svg"
...
# ---------------------------------------------------------------------
# Exterior/concrete from v27 / v35
# ---------------------------------------------------------------------
main_driveway = [
...
# ---------------------------------------------------------------------
# Roof/eaves outline and buildings
# ---------------------------------------------------------------------
roof_pts = [
...
ax.add_patch(Polygon(roof_pts, closed=True, facecolor=house_fill,
                     edgecolor=roof_outline, linewidth=1.6, linestyle="--", zorder=8, alpha=0.55))
...
# ---------------------------------------------------------------------
# v36 polished version of v35
#
# Strategy:
# - Keep the v35 / user purple-overlay topology.
# - Make the hallway read as one cleaner continuous space.
# - Clarify the kitchen / entrance / Bedroom 2 junction.
# - Make laundry/toilet readable without moving them from the right end.
# - Use lighter roof/eaves outline so internal plan is easier to read.
# - Use the .md gently: kitchen/dining as connected, Bedroom 2 long/narrow,
#   laundry/toilet at the right end, lounge connected to sunroom.
# ---------------------------------------------------------------------
label_effect = [pe.withStroke(linewidth=3, foreground="white", alpha=0.85)]

def add_room(label, pts, fill, fontsize=7.0, z=11):
    ax.add_patch(Polygon(pts, closed=True, facecolor=fill, edgecolor=wall_colour,
                         linewidth=1.35, zorder=z))
    if label:
...
# Polished room blocks. These are only small nudges from v35, not a redesign.
...
kitchen_dining = [P(742, 314), P(822, 314), P(822, 496), P(742, 496)]

add_room("Master\nBedroom", master, bed_fill, fontsize=6.9)
add_room("Office", office, bed_fill, fontsize=6.9)
add_room("Bathroom", bathroom, wet_fill, fontsize=6.7)
add_room("Bedroom 2", bedroom2, bed_fill, fontsize=7.1)
add_room("Entrance", entrance, entrance_fill, fontsize=6.7)
add_room("Laundry", laundry, wet_fill, fontsize=6.4)
add_room("Toilet", toilet, wet_fill, fontsize=6.1)
add_room("Hallway", hallway, hall_fill, fontsize=7.0)
add_room("Lounge", lounge, living_fill, fontsize=7.3)
add_room("Kitchen /\nDining", kitchen_dining, kitchen_fill, fontsize=6.9)

# Kitchen/dining divider is light, not a hard visual separation.
ax.plot([P(742, 388)[0], P(822, 388)[0]],
        [P(742, 388)[1], P(822, 388)[1]],
        color=wall_colour, linewidth=1.0, linestyle="--", alpha=0.75, zorder=15)
ax.text(*P(782, 346), "Dining", ha="center", va="center", fontsize=6.1, color="#263b2b",
        zorder=16, path_effects=label_effect)
ax.text(*P(782, 446), "Kitchen", ha="center", va="center", fontsize=6.2, color="#263b2b",
        zorder=16, path_effects=label_effect)

# Cleaner central connector/threshold zone:
# drawn as subtle hallway fill rather than a confusing tiny extra room.
central_connector = [P(758, 500), P(800, 500), P(800, 523), P(758, 523)]
ax.add_patch(Polygon(central_connector, closed=True, facecolor=hall_fill,
                     edgecolor=wall_colour, linewidth=1.15, zorder=12))

# Overdraw important wall lines to create crisp structure, closely following the purple overlay.
def wall(points, lw=1.55, alpha=1.0):
    xs, ys = zip(*[P(x, y) for x, y in points])
    ax.plot(xs, ys, color=wall_colour, linewidth=lw, alpha=alpha,
            solid_capstyle="butt", zorder=16)

# Main walls
wall([(530,485), (608,485), (608,523), (758,523), (758,500), (800,500), (800,523), (890,523), (890,610)])
...
wall([(742,388), (822,388)], lw=1.0, alpha=0.75)
wall([(638,358), (638,485)])
wall([(940,496), (940,610)])
wall([(520,610), (940,610)], lw=1.9)

# Opening cues: use fill colour to create readable gaps/doorways.
def opening(points, colour, lw=3.8):
    xs, ys = zip(*[P(x, y) for x, y in points])
    ax.plot(xs, ys, color=colour, linewidth=lw, solid_capstyle="butt", zorder=17)

# Sunroom to lounge large slider
opening([(637,410), (637,470)], sunroom_fill, lw=5.2)

# Lounge to kitchen/dining large opening
opening([(707,485), (742,485)], hall_fill, lw=4.2)

# Hallway and connector openings
opening([(758,508), (800,508)], hall_fill, lw=4.1)
opening([(800,523), (840,523)], hall_fill, lw=4.0)

# Hallway to master / office / bathroom
opening([(608,506), (640,506)], hall_fill, lw=3.6)
opening([(620,523), (654,523)], hall_fill, lw=3.4)
opening([(698,523), (730,523)], hall_fill, lw=3.4)

# Entrance to kitchen / laundry
opening([(840,500), (882,500)], entrance_fill, lw=3.6)
opening([(890,525), (890,560)], entrance_fill, lw=3.5)

# Re-outline roof/eaves and sunroom.
ax.add_patch(Polygon(roof_pts, closed=True, fill=False, edgecolor=roof_outline,
                     linewidth=1.6, linestyle="--", zorder=18, alpha=0.75))
...
for i, (label, fill, edge) in enumerate(legend_items):
    ...
    ax.add_patch(Rectangle((legend_x, y), 0.45, 0.28,
                           ...
                           linestyle="--" if "Roof" in label else "-", zorder=22))
...
ax.text(PROPERTY_LENGTH / 2, -1.15,
        "Clean traced site plan v36 — polished internal overlay",
